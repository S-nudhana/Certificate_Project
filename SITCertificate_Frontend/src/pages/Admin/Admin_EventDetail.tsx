...
export default function Admin_EventDetail() {
  ...
  const toggleCommentStatus = async (commentId: number, commentDetail: string) => {
    try {
  ...
  const sendMailToProfessor = async (commentDetail: string) => {
    try {
      setIsLoading(true);
      ...
      >
        ...
        >
          ...
          <Text pt="10px" pb="10px">
            เปิดให้ดาว์นโหลดตั้งแต่ {eventData?.event_startDate ? formatDateDMY(new Date(eventData.event_startDate)) : "N/A"}{" "}
            ถึง {eventData?.event_endDate ? formatDateDMY(new Date(eventData.event_endDate)) : "N/A"}
          </Text>
          ...
          <Flex gap={"10px"} alignItems={"center"}>
            ...
            <Text display={excel ? "none" : "flex"}>ไม่พบไฟล์รายชื่อผู้เข้าร่วม</Text>
            <Tooltip
              hasArrow
              placement="right"
        ...
        <Flex ml={["10%", "10%", "0%"]} width={{ base: "90%", md: "50%" }}>
          <Stack spacing={5} w={"full"} pr={"10%"}>
            ...
            <Box width={"100%"} display={comments.length === 0 ? "none" : "block"}>
              {comments &&
                comments.map((item, key) => (
                  <Card key={key} p={"20px"} mb={"20px"} variant={"outline"}>
                    <Flex alignItems={"center"} justifyContent={"space-between"} gap={"10px"}>
                      ...
                      <IconButton
                        ...
                        onClick={() => toggleCommentStatus(item.comment_Id, item.comment_detail)}
                        disabled={isLoading}
                      />
                    </Flex>
            ...
            <Box display={comments.length === 0 ? "flex" : "none"} alignItems={"center"} textAlign={"center"} width={"100%"} height={"15vh"} justifyContent={"center"}>
